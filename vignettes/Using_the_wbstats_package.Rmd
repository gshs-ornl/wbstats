...
#### NOTE: As of October 20, 2018 the GEM Commodities Database was temporarily unavailable on the World Bank API per the World Bank. Please see the GEM Commodities webpage^[<http://data.worldbank.org/developers>]
...
```{r error=TRUE, message=TRUE, warning=TRUE, verbose=TRUE}
library(wbstats)
...
```{r vig-ex-posixct-1, fig.height = 4, fig.width = 7.5, fig.path = "figures/", error=TRUE,  message=TRUE, verbose=TRUE}
library(wbstats)
library(ggplot2)
...
```{r vig-ex-posixct-2, fig.height = 4, fig.width = 7.5, fig.path = "figures/", error=TRUE,  message=TRUE, verbose=TRUE}
library(wbstats)
library(ggplot2)
